...
# WARNINGS ARE SUPPRESSED RIGHT NOW. EXAMINE ISSUE FURTHER
...
The SVM optimization problem can be represented as:

$$\min_{\beta, \beta_0} \frac{1}{2}||\beta||^2 + C\sum_{i=1}^{n}\xi_i$$

Subject to: $y_i(\beta^Tx_i + \beta_0) \geq 1 - \xi_i$ and $\xi_i \geq 0$ for all $i$.

Where:
- $\beta$ and $\beta_0$ define the hyperplane
- $C$ is the cost parameter that controls the trade-off between margin width and misclassification
...
sample = sample(nrow(df), nrow(df) * .60)
...
We'll use a grid search with cross-validation to find optimal values:

```{r}
# Perform 5-fold cross-validation for parameter tuning
# RUNS FOR VERY VERY LONG. FIND WORKAROUND OR BETTER - Used smaller training set
set.seed(456)
svm_tune <- tune(
  svm,
  tv ~.,
...
Now we'll train our final model using the near optimal parameters identified through tuning:
...
3. **Avoiding Overfitting/Underfitting**: 
   ...
   - The gap between training and test error is roughly 7-8% (and shrinks with more data), suggesting the model is neither overfit nor underfit.
